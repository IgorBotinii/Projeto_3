...
void excluirInvestidor(const char *arquivoNome) {
  ...
  while (fgets(linha, sizeof(linha), arquivoOriginal) != NULL) {
    if (strstr(linha, cpf) != NULL) {
      ...
      if (total_usuarios_com_cpf < MAX_USUARIOS) {
        snprintf(usuarios_com_cpf[total_usuarios_com_cpf], TAM_LINHA,
                 "CPF: EXCLUIDO SENHA: EXCLUIDO\n");
      ...
      if (total_usuarios_sem_cpf < MAX_USUARIOS) {
        ...
        usuarios_sem_cpf[total_usuarios_sem_cpf][TAM_LINHA - 1] =
            '\0'; // Garantir que a string seja nula-terminada
  ...
  if (historicoArquivo == NULL) {
    printf("Erro ao abrir o arquivo de historico para escrita! Um novo será "
           "criado.\n");
  ...
  fprintf(historicoArquivo, "CPF: %s\tEXCLUIDO: %s\n", cpf,
          cpf); // Registrando o CPF que foi excluído
...
void cadastrar_criptomoeda() {
  ...
  cotacao_inicial =
      solicitar_float("Digite a cotação inicial da criptomoeda: ");
  ...
  if (fprintf(
          arquivo,
          "NOME: %s | COTAÇÃO: %.2f | TAXA COMPRA: %.2f | TAXA VENDA: %.2f\n",
...
void excluir_criptomoeda() {
  ...
  if (encontrada) {
    ...
    if (remove("criptomoedas.txt") != 0 ||
        rename(nome_arquivo_temp, "criptomoedas.txt") != 0) {
    ...
    remove(
        nome_arquivo_temp); // Remove o arquivo temporário, pois não foi usado
  }
}
...
int login() {
  ...
  while (1) {
    ...
    int loginValido = 0;
    while (fscanf(arquivo, "CPF: %s SENHA: %s", cpfArquivo, senhaArquivo) !=
           EOF) {
    ...
    if (loginValido) {
      ...
      printf(
          "CPF ou senha incorretos. Tente novamente ou digite 0 para sair.\n");
    }
  }
}
